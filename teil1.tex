...
Die Suche nach dem Nullelement stellt sich als trivial dar. Wir suchen ein Nullelement $N$, so dass $L\ff N = N$ sowie $N\ff L = N$ für alle $L\subseteq X^*$ gilt. Man erkennt leicht, dass die leere Menge $\emptyset$ ein solches Nullelement darstellt, da $\emptyset \ff L = \emptyset$ und $L\ff \emptyset = \emptyset$ gelten.\\\\
...
Aus der Definition \ref{continuecup} und aus Eigenschaft \ref{cupeig} folgt:
\begin{eigen}\label{fresh}
$L\subseteq W \to L\ff V \subseteq W \ff V$
\end{eigen}

Betrachten wir nun $L\cap V \subseteq L$ und $L\cap V \subseteq V$, dann haben wir mit Eigenschaft \ref{fresh} die Inklusion:
\begin{eigen}
...
Es gilt keine Monotonie im Vorderglied.
\begin{eigen}
$L\subseteq W \not\to V\ff L \subseteq V \ff W$
\end{eigen}
...
der linken Seite der Gleichung um Wörter aus $W$ handelt die man vergleicht mit Wörtern aus $W^2$. \\
Demnach treten hier Eigenschaften nur auf wenn $W$ eine ganz spezielle Gestalt hat, nämlich $W\subseteq W^2$ oder $W\supseteq W^2$

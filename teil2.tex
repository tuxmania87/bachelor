...
\underline{$\pref(V)\ff W$}\\
1. Fall: $w\in W \cap \pref(V) \to w\in \pref(V)\ff W$\\
2. Fall: $w\in W\backslash \pref(V) \to $muss im Einzelfall geprüft werden.\\
$\to \pref(V)\ff W = \underbrace{W\cap \pref(V)}_A \cup  \textit{pref(V)}\backslash A \ff W$\\\\


\underline{$W\ff \pref(V) = W \cap \pref(V)$}\\
"'$\rightarrow$"'\\
$\bigcup_{w\in W} \min_{\sqsubseteq} \{v:v\in \pref(V) \wedge w\sqsubseteq v\}$\\
$v\in \pref(V) \wedge w\sqsubseteq v \to w\in \pref(V)$\\
$\bigcup_{w\in W} \{w\in \pref(V)\} = W\cap \pref(V)$\\
\\
%"'$\leftarrow$"'\\
...
\newpage
$$V\subseteq X^*\backslash W\cdot X^*$$

$$W\ff V = \emptyset$$
$$W\ff pref(V) = \emptyset$$
$$W\cdot X^*\ff V = \emptyset$$
$$V\ff W\cdot X^* = V\ff W = \max(V)\ff \min(W)$$
